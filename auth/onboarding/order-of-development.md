...
3. Build [Email verification - start](ui-reference/email_verification_start/code.html), including validation, submission, and the neutral response for addresses that may already exist.
4. Build [Email verification - wait](ui-reference/email_verification_wait/code.html), including resend limits and change-email navigation.
...
6. Handle the opened link:
    ...
    - Without A, show [Email verification - manual code input](ui-reference/email_verification_manual_code_input/code.html). Submit B+C only when the user activates Verify email.
7. Add [incorrect code](ui-reference/email_verification_incorrect_code/code.html), [attempt limit](ui-reference/email_verification_attempt_limit/code.html), and [unusable link](ui-reference/email_verification_link_unusable/code.html). A replacement email supplies a new link and code; old and new values cannot be combined.
...
9. Build [Passkey setup - start](ui-reference/passkey_setup_start/code.html) and invoke the browser or platform credential UI from its Create passkey action.
...
11. Add [Passkey setup - failed](ui-reference/passkey_setup_failed/code.html) with Try again and Do this later.
12. Add [Passkey setup - unsupported device](ui-reference/passkey_setup_unsupported_device/code.html). Allow deferral and the documented email sign-in path; recheck support only when the device environment changes.

## 4. Optional identity verification

13. Build [Identity verification - start](ui-reference/identity_verification_start/code.html), with separate email and passkey statuses, Start identity check, and Skip for now.
...
15. Use the shared processing screen to read the returned images. For unreadable images, show [Identity verification - unreadable document](ui-reference/identity_verification_document_unreadable/code.html) and reopen the plugin on retry.
16. Build [Identity verification - review details](ui-reference/identity_verification_review_details/code.html). Let the user correct extracted data before submission; extraction is not an identity decision.
17. Use the shared processing screen for submission. On interruption, show [Identity verification - submission failed](ui-reference/identity_verification_submission_failed/code.html). Check the server outcome before retrying an uncertain submission.
18. Use the shared processing screen or a persistent pending state while the identity provider decides. Show [Identity verification - success](ui-reference/identity_verification_success/code.html) only after an approved result.
